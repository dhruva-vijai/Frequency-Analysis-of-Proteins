    ...
    for i in d:
        for j in di:
            if d[i] in di[j]:
                ...
                di[ind]=i
    for i in di:
        print(str(i),str(len(di[i])))


def prediction(string):
    k=read_uniprot.reader(string)
    m=[]
    for i in k:
        m.append(frequencies(k[i]))
    clf=Mean_Shift()
    return(clf.predict(m))
